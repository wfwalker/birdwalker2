...
echo "tripcreate" && php ./tripcreate.php | grep -q error && echo "tripcreate ERROR" && exit 1
...
echo "photoindex" && php ./photoindex.php | grep -q error && echo "photoindex ERROR" && exit 1
...
echo "photodetail" && curl -s http://localhost/~walker/birdwalker2/photodetail.php?id=7861 | grep error && echo "photodetail.ERROR" && exit 1
echo "sightingdetail" && curl -s http://localhost/~walker/birdwalker2/sightingdetail.php?id=7861 | grep error && echo "sightingdetail.ERROR" && exit 1
echo "everything ok"
