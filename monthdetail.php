...
$concatenation = "concat(Year(trips.Date), lpad(Month(trips.Date), 2, '0'))";
...
if ($request->getYear() != "")
{
    ...
    "SELECT min(" . $concatenation . ") FROM sightings, trips WHERE sightings.trip_id=trips.id AND " . $concatenation . " > '" . $request->getYear() . $request->getMonth() . "'");
	
	$nextYear = substr($nextMonthInfo, 0, 4);
	$nextMonth = substr($nextMonthInfo, 4, 2);
	
	$prevMonthInfo = performCount("Find Previous Month",
    "SELECT max(" . $concatenation . ") FROM sightings, trips WHERE sightings.trip_id=trips.id AND " . $concatenation . " < '" . $request->getYear() . $request->getMonth() . "'");
