...
<?
	...
	function navTrailBirds()
	{
		...
		if ($this->getFamilyID() != "")
		{
			...
			$items[] = $orderRequest->command($orderInfo["LatinName"]);
		}
		...
		if ($this->getSpeciesID() != "")
		{
			...
			$items[] = $familyRequest->command($familyInfo["LatinName"]);
		}
	...
	function navTrailLocations()
	{
		...
		if ($this->getCounty() != "")
		{
			...
			$items[] = $stateRequest->command($stateInfo["Name"]);
		}
		...
		if ($this->getLocationID() != "")
		{
			...
			$items[] = $countyRequest->command($this->getCounty() . " county");
		}
	...
	function navTrailTrips()
	{
		...
		if ($this->getMonth() != "")
		{
			...
			$items[] = $yearRequest->command($this->getYear());
		}
		...
		if ($this->getTripID() != "")
		{
			...
			$items[] = $monthRequest->command(getMonthNameForNumber($this->getMonth()));
		}
